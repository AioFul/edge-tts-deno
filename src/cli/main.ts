...
import { SubtitleGeneratorFactory } from "../utils/subtitle.ts";
...
async function runTTS(args: CLIArgs) {
  try {
    ...
    // 创建字幕生成器
    const subtitleGenerator = args.writeSubtitles ? SubtitleGeneratorFactory.create({
      wordsPerCue: args.wordsInCue,
    }) : null;
    ...
    for await (const chunk of client.stream()) {
      ...
      } else if (chunk.type === "WordBoundary" && subtitleGenerator) {
        // 添加字幕片段
        subtitleGenerator.addCue(chunk);
      }
    }

    // 合并并写入字幕文件
    if (args.writeSubtitles && subtitleGenerator) {
      subtitleGenerator.mergeCues(args.wordsInCue);
      await Deno.writeTextFile(args.writeSubtitles, subtitleGenerator.generate());
    }
